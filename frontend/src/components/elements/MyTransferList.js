...
function handleSelect (items, setAge, side, setRight, setLeft, right, left) {
  if (side === 'right') {
    setRight(right.filter(item => item.name === items))
...
export default function MyTransferList (inputProps) {
  ...
  const { onNameChange, subjects, mySubjects, changeSubjects } = inputProps
  ...
  const customList = (items, input, side) => (
                      ...
                      <div>
                        <FormControlLabel
                                ...
                                control={<Select
                                    ...
                                    variant="outlined"
                                    onChange={event => handleSelect(event.target.value, setAge, side, setRight, setLeft, right, left)}
                                    displayEmpty
                                    fullWidth
  ...
  return (
    <Grid container spacing={2} justify="center" alignItems="center" className={classes.root}>
      <Grid item>{customList(left, input, 'left')}</Grid>
      <Grid item>
      ...
      <Grid item>{customList(right, input, 'right')}</Grid>
    </Grid>
  )
}
